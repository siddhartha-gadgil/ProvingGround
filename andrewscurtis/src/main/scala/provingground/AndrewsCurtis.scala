...
/*
  ...
  object Chain {
    ...
    @annotation.tailrec
    def backprop(
        chain: Chain, mult: Double, d: DynDstbn, accum: DynDstbn): DynDstbn =
      chain match {
   ...
   */
  def backpropdstbn(
      chains: Set[Chain], feedback: FiniteDistribution[Vert], d: DynDstbn) = {
//    val empty = FiniteDistribution[DynObj](Set())
  ...
  def dstbnFeedback(
    ...
    val fdbkpmf =
      fdbkrawpmf map ((x) =>
            Weighted(x.elem, x.weight - (fdbkpmftot / fdbkrawpmf.size)))
  ...
  @annotation.tailrec
  def chainGenCutoff(
      chains: Set[Chain], d: DynDstbn, cutoff: Double): Set[Chain] = {
   ...
   */
  def tuneFlowCutoff(
      d: DynDstbn, bgwt: Vert => Double, epsilon: Double, cutoff: Double) = {
