...
//    val empty = FiniteDistribution[DynObj](Set())
  ...
  def dstbnFeedback(presdstbn : FiniteDistribution[Vert] , bgwt : Vert => Double) = {
    ...
    val fdbkpmf = fdbkrawpmf map ((x) => Weighted(x.elem, x.weight - (fdbkpmftot/fdbkrawpmf.size)))
...
*/
  def initDstbn(pthCntn: Double) =  DynDst(FiniteDistribution(Set(Weighted(nullpres, 1.0))), FiniteDistribution.empty[Presentation], pthCntn)
