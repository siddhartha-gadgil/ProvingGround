...
object AndrewsCurtisModel {
  ...
  implicit object ACMoveFreqFormat
      extends Format[FiniteDistribution[ACMoveType]] {
    def reads(js: JsValue): JsResult[FiniteDistribution[ACMoveType]] = {
      val pmf = for ((x, y) <- js.as[Map[String, Double]]) yield
        Weighted(ACMoveType.fromString(x), y)
  ...
  def allMoves(pres: Presentation): MoveType => List[Move] = {
    ...
    case RtMultMv =>
      val n = pres.rank
      (for (j <- 0 to n - 1; k <- 0 to n - 1 if j != k) yield
        RtMult(j, k)).toList
    case LftMultMv =>
      val n = pres.rank
      (for (j <- 0 to n - 1; k <- 0 to n - 1 if j != k) yield
  ...
  val moves = (pres: Presentation, mvtyp: ACMoveType) =>
    allMoves(pres)(mvtyp).toSet map ((mv: Move) => mv(pres))

  val learner = new MoveLearner(MoveTypeList, moves)

  import learner._

  case class ACparams(cutoffexp: Int = 7,
                      stableLevelexp: Int = 7,
                      stablSteps: Int = 25,
                      outerSteps: Int = 25,
                      epsilonexp: Int = 7,
                      presCntnSc: Int = 70,
    ...
    def baseweights =
      presentationWeight(_: Presentation, presCntn: Double, wrdCntn: Double)

    implicit def feedback(fd: FiniteDistribution[Presentation]) =
      fd.rawfeedback(baseweights)

    lazy val learnLoop = LearningLoop(cutoff: Double,
                                      stableLevel: Double,
                                      stablSteps,
                                      outerSteps: Int,
                                      epsilon: Double)(feedback)
  ...
  val defaultdstbn = DynDst(
      FiniteDistribution(Set(Weighted(nullpres, 1.0))), unifmoves, 0.7)
  /*
    ...
    Form(
        mapping(
            ...
            "epsilon" -> number
              ...
              LearningLoop(a, b, c, d, e)(fd))((x) =>
              LearningLoop.unapply(x).map(roundoff)))
