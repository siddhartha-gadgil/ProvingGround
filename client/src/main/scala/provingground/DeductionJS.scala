...
//    val cons = div(refresh, info, selected).render
    ...
    def xc(x: Double) = (xScale * x).toInt
     ...
     jsDiv.appendChild(
       div(
         h1("Dynamics and Learning for deduction"),
         plotDiv, selected, div(all.style := "clear: both;"),
         refresh,
          info, queryDiv,
      ...
      for (
        ...
        ((val1, val2), index) <- (points zip points.tail).zipWithIndex
          ...
          line(onclick := {() => {
            ...
            x1 := xc(index.toDouble), y1 := yc(val1), x2 := xc(index.toDouble + 1 ), y2 := yc(val2),
            stroke := "blue", strokeWidth := 3).render
      }
      ...
      def showFD() = {
        ...
        plot.appendChild(
          rect(
            ...
            fill := "grey", fillOpacity := "0.1").render
        )
        ...
        Ajax.get("../terms-data").onSuccess{
          ...
          Ajax.get("../terms-time-series").onSuccess{
            case xhr =>
              ...
              val items = tsList.sortBy(_._2.last).reverse map ((lv) => katex(lv._1)) map (li(_))
