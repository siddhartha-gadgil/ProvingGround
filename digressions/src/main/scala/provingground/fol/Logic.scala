...
object Logic {
  ...
  val varstream: Stream[Var] =
    (Stream.from(0)) map ((x: Int) => VarSym((x + 'a').toChar.toString))
  ...
  trait Formula extends Expression {
    ...
    lazy val sentence =
      ...
      else (freeVars :\ this)(UnivQuantFormula(_, _))
  ...
  implicit def eqFmla(pq: Eq): AtomFormula =
    AtomFormula(BinRel("="), List(pq.p, pq.q))
...
// This is fixed for a given interpretation as the stream varies
  ...
  trait Model[A] extends PrimMap[A] {
    ...
    private def changeVar(x: Var, a: A, z: Mbar): Mbar =
      (y: Var) => if (x == y) a else z(y)

    /** Variation of z along x */
    def variation(x: Var, z: Mbar): Stream[Mbar] = M map (changeVar(x, _, z))

    private def varFormula(x: Var, z: Mbar, phi: Formula): Stream[Boolean] =
      variation(x, z) map (sec(phi, _))
