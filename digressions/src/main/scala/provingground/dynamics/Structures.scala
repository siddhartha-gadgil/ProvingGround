...
object Structures {
  ...
  def setProd[A, B](s: Set[A], t: Set[B]): Set[(A, B)] =
    (for (x <- s; y <- t) yield (x, y)).toSet

  /** Product of a List of Sets */
  def listProd[A](ls: List[Set[A]]): Set[List[A]] =
  ...
  def indexSetProd[A](ss: Set[Set[A]]): Set[Set[A]] =
    if (ss.isEmpty) Set(Set())
    else {
      for (s <- ss.head; t <- indexSetProd(ss.tail)) yield (t + s)
    }

  /** Returns fixed point of a function. Loops forever if there is none */
  def fixedPoint[A](x: A, f: A => A): A =
    if (f(x) == x) x else fixedPoint(f(x), f)
  ...
  def allMaps[A, B](S: Set[A], T: Set[B]): Set[PartialFunction[A, B]] = {
    if (S.isEmpty) Set((Map.empty: PartialFunction[A, B]))
      ...
      (for (t <- T; f <- allMaps(S.tail, T)) yield
        (Map(S.head -> t) orElse f)).toSet
    }
  }

  /** Returns f^{-1}(b) for a partial function f*/
  def invSet[A, B](b: B, dom: Set[A], f: PartialFunction[A, B]) =
    dom filter (f.lift(_) == Some(b))
  ...
  def transClosure[A](S: Set[(A, A)]): Set[(A, A)] =
    fixedPoint(S, (T: Set[(A, A)]) => T union edgePairs(T))
  ...
  /** Symmetric relation */
  class SymRelation[A](relSet: Set[(A, A)])
      extends Relation[A](relSet: Set[(A, A)]) {
    override lazy val equiv = new EquivRelation(transSet)
  }

  /** Equivalence relation */
  class EquivRelation[A](relSet: Set[(A, A)])
      extends SymRelation[A](relSet: Set[(A, A)]) {
  ...
  def wordStream[A](gen: Set[A]): Stream[List[A]] =
    wordSetStream[A](gen).flatten
...
// Enumerating in a groupoid multiplying all that are posssible.
    ...
    private def spanNext[A <: Groupoid[A]](
        gens: Stream[A], startSet: Set[A], n: Int = 1) = {
    ...
    def stream[A <: Groupoid[A]](gens: Stream[A]): Stream[A] =
      (Stream.from(1)) flatMap (span(gens, _))

    /** Span of generators without repetition */
    def cleanStream[A <: Groupoid[A]](gens: Stream[A]): Stream[A] =
      (Stream.from(1)) flatMap (spanNew(gens, _))
  }
