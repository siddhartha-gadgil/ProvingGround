...
object StackedEvolver {
  ...
  trait BaseEvolver[A] {
    ...
    /** Initial set*/
    val init: Set[A] =
      if (gens.hasDefiniteSize) gens.toSet else (gens take 1).toSet
    ...
    private def nxt(gens: Stream[A], S: Set[A], n: Int) = {
      val genSet: Set[A] =
        ((gens take n).toSet union S) filter (weight(_, n) <= n)
    ...
    val newSetStream: Stream[Set[A]] =
      (setStream, setStream.tail).zipped.map(_ -- _)
  ...
  class SimpleEvolver[A](val gens: Stream[A], span: A => Set[A])
      extends BaseEvolver[A] {
    override def mixinSpan(S: Set[A], n: Int): Set[A] =
      S flatMap ((a: A) => span(a))
  }
  ...
  trait AddinEvolver[A] extends BaseEvolver[A] with Span[A] {
    override def mixinSpan(S: Set[A], n: Int): Set[A] =
      super.mixinSpan(S, n) union span(S, n)
  }
  ...
  trait SpanUnion[A] extends Span[A] {
    ...
    def span(S: Set[A], n: Int) =
      spans.toSet flatMap ((s: Span[A]) => s.span(S, n))
  }
  ...
  trait SpanMultiProd[A] extends Span[A] {
    ...
    def span(S: Set[A], n: Int): Set[A] =
      (setProd(S, S) collect (multiProd)) reduce (_ union _)
  }
...
object GeneticEvolver {
  ...
  def listMap[A](f: A => A) = { (l: List[A]) =>
    if (l.isEmpty) l else f(l.head) :: l
  }
  ...
  case class RecTree[A](root: A, left: BinaryTree[A], right: BinaryTree[A])
      extends BinaryTree[A]

  def treeMap[A](prod: (A, A) => A) = {
    def lr(left: BinaryTree[A], right: BinaryTree[A]) =
      RecTree(prod(left.root, right.root), left, right)
    lr(_, _)
  }
  ...
  def pickRandByWeight[A](l: List[A], weight: A => Int)(
      implicit rand: scala.util.Random): A = {
    val total = (l map (weight(_))).sum
    pickByWeight(l, weight, rand.nextInt(total))
  }

  def sampleByWeight[A](l: List[A], weight: A => Int, n: Int)(
  ...
  case class Markov[A](dyn: Set[A] => Set[A]) {
    ...
    def ++(dmap: Set[A] => Set[A]) =
      Markov((s: Set[A]) => dyn(s) union dmap(s))
  ...
  class MarkovEvolver[A](val base: Set[A],
    ...
    def this(base: Set[A], dyn: Set[A] => Set[A]) =
      this(base, Markov(dyn), Set.empty: Set[A])

    def this(base: Set[A], dyn: Set[A] => Set[A], coreSet: Set[A]) =
      this(base, Markov(dyn), coreSet)
...
object Evolver {
  ...
  class Gen(dyn: => (Set[Term] => Set[Term]),
            state: => Set[Term],
            mapping: Term => Term = { (x: Term) =>
              x
            },
            outbox: Outbox[Term] = VanishBox[Term])
      extends AtomicTerm {
...
//     println(nextChar(usedChars((state))))
  ...
  trait Evolver[A] extends DynSys[A] {
    ...
    def step: Set[A] => Set[A] =
      (S: Set[A]) => purge(generate(S) union inbox.pullAll)

    @tailrec final def getNow(S: Set[A], n: Int): Set[A] =
      if (n < 1) S else getNow(step(S), n - 1)
    ...
    def seekNow(S: Set[A], p: A => Boolean, n: Int) =
      seekNowWithState(S, p, n)._1
  ...
  object Dyn {
    ...
    def apply[A](pf: PartialFunction[A, A]): Dyn[A] =
      Dyn((s: Set[A]) => s collect pf)
  }
