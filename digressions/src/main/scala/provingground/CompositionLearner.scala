...
class CompositionLearner[V, M](movetypes: List[M], moves: (V, M) => Set[V]) {
  ...
  /**
    ...
    *
    */
  case class DynDst(vrtdst: FiniteDistribution[V],
                    edgdst: FiniteDistribution[M],
                    cntn: Double) {
    ...
    /**
      ...
      */
    def --(that: DynDst) =
      DynDst(vrtdst ++ (that.vrtdst * (-1)),
             edgdst ++ (that.edgdst * (-1)),
             cntn - that.cntn)
...
//        edgdst.entropyView(lines), cntn)
  ...
  object DynDst {
    ...
    /**
      ...
      */
    def empty =
      DynDst(FiniteDistribution.empty[V], FiniteDistribution.empty[M], 0)
  }
  ...
  case class RecChain(head: V, tail: Chain, mvtyp: M) extends Chain {
    ...
    /**
      ...
      */
    def prob(d: DynDst): Double =
      tail.prob(d) * d.edgdst(mvtyp) * d.cntn / mult
  }
  ...
  /**
    ...
    */
  @tailrec final def propagate(chains: Set[Chain],
                               prune: Chain => Boolean,
                               prev: Set[Chain] = Set.empty): Set[Chain] =
    if (prev == chains) chains
    else propagate(prunednextgen(chains, prune), prune, chains)
  ...
  @tailrec final def backpropchain(initdstbn: DynDst,
                                   error: Double,
                                   accum: DynDst = DynDst.empty,
                                   chain: Chain): DynDst = chain match {
...
//      val headdst = accum addV(head, headWt) addC(cntnWt)
  ...
  /**
    ...
    */
  case class LearningLoop(cutoffExp: Double,
                          stableLevelExp: Double,
                          innerSteps: Int,
                          outerSteps: Int,
                          epsilonExp: Double)(feedback: Feedback) {
    ...
    /**
      ...
      */
    @tailrec final def innerlearn(d: DynDst,
                                  steps: Int = innerSteps,
                                  isStable: Boolean = false,
                                  chains: Set[Chain]): DynDst =
    ...
    /**
      ...
      */
    @tailrec final def outerlearn(d: DynDst,
                                  steps: Int = outerSteps,
                                  isStable: Boolean = false): DynDst =
      if (steps < 1 || isStable) d
      else {
