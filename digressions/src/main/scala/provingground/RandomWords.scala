 ...
 */
  ...
  case class BinOpSym[E, N](name: N, op: (E, E) => E) extends Symbol[E]{self =>
    def apply(a: E, b: E) = op(a,b)

    def apply(a: Word[E], b: Word[E]) = BinOpWord(this, a, b)

    def assoc(a: Word[E], b: Word[E]) =  (a, b) match {
      case (x : AssocWord[E, N], y: AssocWord[E, N]) if x.op == self && y.op == self => AssocWord(self, x.elems ::: y.elems)
      case (x : AssocWord[E, N], y) if x.op == self => AssocWord(self, x.elems :+ y)
      case (x, y : AssocWord[E, N]) if y.op == self => AssocWord(self, x +: y.elems)
      case _ => apply(a, b)
    }
  }
  ...
  case class BinOpWord[E, N](op: BinOpSym[E, N], first: Word[E], second: Word[E]) extends Word[E]{
  ...
  case class AssocWord[E, N](op: BinOpSym[E, N], elems: List[Word[E]]) extends AggregateWord[E]{
    ...
    def terms = (for (n <- 1 to elems.length -1) yield List(AssocWord(op, elems take n), AssocWord(op, elems drop n))).flatten.map (_.simplify)
  }

  def recprob[E](m: FiniteDistribution[Symbol[E]])(w: Word[E]) : Double = w match {
    case l : Letter[_] => m(l)
    case word @ BinOpWord(op, first, second) => m(op) * m(first) * m(second) + m(word)
    case word @ UnOpWord(op, elem) => m(op) * m(elem) + m(word)
    case agg : AggregateWord[_] => (agg.terms map (recprob(m)(_))).sum
  }

  def partial[E](m: FiniteDistribution[Symbol[E]])(w: Word[E], x: Symbol[E]): Double = (x, w) match {
    case (a, b) if a ==b => 1
  ...
  def evolution[E](supp: Set[Word[E]]) = {
    def pushforward(m: FiniteDistribution[Symbol[E]]) = supp map ((w) => Weighted(w, recprob(m)(w)))

    def pushmass(m: FiniteDistribution[Symbol[E]]) = (pushforward(m) map (_.weight)).sum

    def forward(m: FiniteDistribution[Symbol[E]]) = FiniteDistribution((pushforward(m) map (_.scale(1.0/pushmass(m)))))
    ...
    def back(m: FiniteDistribution[Symbol[E]])(vect: FiniteDistribution[Word[E]]) = {
      ...
      def xterms(x: Symbol[E]) = (for (Weighted(w, p) <- pushforward(m)) yield p * partial(m)(w, x)).sum
