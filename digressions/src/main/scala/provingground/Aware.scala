...
object Aware {
  ...
  case class Reduction[A, B](
      reduction: Task, goal: Task, solver: Solution[A] => Solution[B])
      extends Response
  ...
  trait SeekerResource {
    ...
    def receiver(asker: ActorRef,
                 cntx: ActorContext,
                 myself: ActorRef): PartialFunction[Any, Unit] = {
      case task: Task =>
        resource(task) match {
    ...
    def fromStream[A](s: Stream[A]): SeekerResource = new SeekerResource {
      val resource: PartialFunction[Task, Either[SeekerResponse, Response]] = {
        case FindInStream(seeker, p, n: Int) if seeker == s =>
          ...
          ((s take n).find(p) map ((a: A) => Right(Solution(a, task))))
            .getOrElse(
              Left(SeekerResponse(NotFound(task), fromStream(s drop n))))
      }
    }

    def fromSeeker[A, S](s: Seeker[A, S]): SeekerResource =
      fromSeeker(s, s.base)
  ...
  case class StringDescription[A](subject: A, description: String)
      extends Description[A]
  ...
  case class NamedRelation[A, B](first: A, second: B, name: String)
