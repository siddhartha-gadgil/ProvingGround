...
object TheoryTypes {
  ...
  class ExpressionParser(binOps: List[String],
    ...
    def realSym: Parser[Term] = floatingPointNumber ^^ {
      case x => RealSym(x.toDouble)
    }
    ...
    def equality: Parser[Term] = noRelTerm ~ "=" ~ term ^^ {
      case lhs ~ "=" ~ rhs => Equality(lhs, rhs)
    }

    def subScript: Parser[Term] = sym ~ "_" ~ simpleTerm ^^ {
      case lhs ~ "_" ~ rhs => SubScript(lhs, rhs)
    }

    def supScript: Parser[Term] = sym ~ "^" ~ simpleTerm ^^ {
      case lhs ~ "^" ~ rhs => SupScript(lhs, rhs)
    }

    def binOpTerm(op: String): Parser[Term] =
      noOpTerm ~ literal(op) ~ noRelTerm ^^ {
        case lhs ~ (_: String) ~ rhs => BinOpTerm(op, lhs, rhs)
      }

    def binRelTerm(op: String): Parser[Term] =
      noRelTerm ~ literal(op) ~ term ^^ {
        case lhs ~ (_: String) ~ rhs => BinOpTerm(op, lhs, rhs)
      }

    def bigOpTerm(op: String): Parser[Term] =
      literal(op) ~ simpleTerm ~ noRelTerm ^^ {
        case (_: String) ~ (lhs: Expression) ~ (rhs: Expression) =>
          BigOpTerm(op, lhs, rhs)
      }

    def application: Parser[Term] = simpleTerm ~ term ^^ {
      case f ~ a => Apply(f, a)
    }

    def func: Parser[Term] = noRelTerm ~ "->" ~ term ^^ {
    ...
    def binOpParse: Parser[Term] =
      (binOps map (binOpTerm(_))).foldLeft(nullTerm)((s, t) => s | t)

    def bigOpParse: Parser[Term] =
      (bigOps map (bigOpTerm(_))).foldLeft(nullTerm)((s, t) => s | t)

    def binRelParse: Parser[Term] =
      (binRels map (binRelTerm(_))).foldLeft(nullTerm)((s, t) => s | t)

    def braces: Parser[Term] = "{" ~ expression ~ "|" ~ term ~ "}" ^^ {
      case "{" ~ index ~ "|" ~ arg ~ "}" => Braces(index, arg)
    }

    def parenTerm: Parser[Term] = "(" ~ term ~ ")" ^^ {
      case "(" ~ t ~ ")" => t
    }

    def texBraces: Parser[Term] = "{" ~ term ~ "}" ^^ {
      case "{" ~ arg ~ "}" => arg
    }
    ...
    def colon: Parser[Expression] = term ~ ":" ~ term ^^ {
      case obj ~ ":" ~ typ => Colon(obj, typ)
    }

    def typedTerm: Parser[TypedTerm] = "(" ~ term ~ ":" ~ term ~ ")" ^^ {
      case "(" ~ obj ~ ":" ~ typ ~ ")" => TypedTerm(obj, typ)
    }

    def lambda: Parser[Term] = typedTerm ~ "->" ~ term ^^ {
      case arg ~ "->" ~ value => LambdaTerm(arg, value)
    }
