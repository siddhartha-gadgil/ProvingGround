...
//  case class FuncPositive(func : RealFunc, domain: Interval) extends ConstantTyp with Func
  ...
  case class FuncBound(func: RealFunc,
                       ...
                       bound: Real,
                       sign: Sign)
      extends ConstantTyp with Func with Bound
  ...
  case class GlueFuncPositive(func: RealFunc, domain: Interval, pfs: Set[Term])
      extends ConstantTerm with Func {
    split(domain) map
    ((j) =>
          // j is a half interval
          {
            val thispfs =
  ...
  case class MVTMidPositive(func: RealFunc,
    ...
    val left =
      mid - ((domain.width / 2) * derUpper.bound) // lower bound for value at left endpoint
    val right =
      mid + ((domain.width / 2) * derLower.bound) // lower bound for value at right endpoint.
    assert(mid >= 0)
    assert(left >= 0)
    assert(right >= 0)
...
//  case class FuncPositiveCube(func: RealMultiFunc, domain: Cube) extends ConstantTyp with MultiFunc
  ...
  case class MultiFuncBound(func: RealMultiFunc,
                            ...
                            bound: Real,
                            sign: Sign)
      extends ConstantTyp with MultiFunc with Bound
  ...
  case class RestrictedMultiFuncBound(domain: Cube, fb: MultiFuncBound)
    ...
    for (i <- 0 to domain.size - 1) yield
      assert(fb.domain(i).lower <= domain(i).lower &&
  ...
  case class FaceBound(func: RealMultiFunc,
                       ...
                       face: Map[Index, Sign],
                       sign: Sign)
      extends ConstantTyp with Bound
