...
class FunctionCombinator[F](add: (F, F) => F, sum: F => Option[(F, F)],
  ...
  def derivative(table : F => Option[F]) : F =>  Option[F] = {
    ...
    def quotient(g: F) = for ((x, y) <- quot(g); a <- derivative(table)(x); b<- derivative(table)(y)) yield 
      (a * y - b * x)/ (y * y)
    
    def chain(g: F) = for ((x, y) <- composition(g); a <- derivative(table)(x); b<- derivative(table)(y)) 
      yield (b * (a circ y))
    
    (f: F) => table(f) orElse sumrule(f) orElse leibnitz(f) orElse quotient(f) orElse chain(f)
  }
  
  def rationalApprox(table : F => Interval[Rational] => Option[Interval[Rational]]) : 
  F =>  Interval[Rational] => Option[Interval[Rational]] = {
    def sumIntvl(g: F)(j: Interval[Rational]) = for ((x, y) <- sum(g); 
      a <- rationalApprox(table)(x)(j); b<- rationalApprox(table)(y)(j)) yield (a + b)
    
    def prodIntvl(g: F)(j: Interval[Rational]) = for ((x, y) <- prod(g); 
      a <- rationalApprox(table)(x)(j); b<- rationalApprox(table)(y)(j)) yield (a * b)
    
    def quotIntvl(g: F)(j: Interval[Rational]) = for ((x, y) <- quot(g); 
      a <- rationalApprox(table)(x)(j); b<- rationalApprox(table)(y)(j);
      bound <- Try(a/b).toOption) yield (bound)
    
    def compositionIntvl(g: F)(j: Interval[Rational]) = for ((x, y) <- sum(g); 
