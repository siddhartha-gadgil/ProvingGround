...
class LeanParser(mods: Vector[Modification]){
  import LeanParser._
  import LeanToTermMonix.{RecIterAp, getRec, applyFuncWit, applyFuncWitFold, isPropnFn, parseWork}
...
def parseVec(
    vec: Vector[Expr],
    vars: Vector[Term]): Task[Vector[Term]] =
  vec match {
...
def withAxiomSeq(
    axs: Vector[(Name, Expr)]): Task[Unit] =
axs match {
  case Vector() => Task(())
  case (name, ty) +: ys =>
    for {
