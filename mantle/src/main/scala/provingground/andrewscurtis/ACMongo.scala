...
object ACMongo extends ACWriter {
  ...
  def getFutOptFDMStep(name: String, loops: Int) = {
    val selector = BSONDocument("name" -> name, "loops" -> loops)
    moveWeightsDB.find(selector).cursor[ACMoveWeights]().headOption map ((opt) =>
          opt map (_.fdM))
  }
  ...
  def getFutStartData() = {
    val entries =
      actorsDB.find(BSONDocument()).cursor[BSONDocument]().collect[Vector]()
    entries map ((vec) =>
          (vec map (_.getAs[String]("start-data") flatMap ((d) =>
                        uread[List[StartData]](d).headOption))).flatten)
  }
...
//    }
  ...
  def thmSaveCSV(thms: Vector[ACThm])(
...
case class ACMoveWeights(
    name: String, fdM: FiniteDistribution[AtomicMove], loops: Int)
