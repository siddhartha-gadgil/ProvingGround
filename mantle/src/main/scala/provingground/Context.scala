...
object Contexts {
  ...
  case class DefnEqual(lhs: Term, rhs: Term, freevars: List[Term] = List())
      extends DefnEquality {
    def map(f: Term => Term) = DefnEqual(f(lhs), f(rhs), freevars map (f))

    def apply(substitutions: Term => Option[Term]) =
      DefnEqual.instantiate(substitutions)(this)

    def apply(substitutions: PartialFunction[Term, Term]) =
      DefnEqual.instantiate(substitutions.lift)(this)
  }
  ...
  object DefnEqual {
    ...
    def instantiate(substitutions: Term => Option[Term])(
        dfn: DefnEqual): Option[DefnEquality] = {
  ...
  case class Defn(
    ...
    def map(f: Term => Term, F: Typ[Term] => Typ[Term]) =
      Defn(lhsname, F(typ), f(rhs), freevars map (f))
  ...
  trait Context[+U <: Term with Subs[U], V <: Term with Subs[V]] {
    ...
    /**
      ...
      *
      */
    def foldinSym(tp: Typ[V])(name: AnySym) =
      foldin(tp)(apply(tp).symbObj(name))
    ...
    def dfn(x: Term, y: Term, local: Boolean = true) =
      DefnMixin(Defn(termSymbol(x), y), this)

    def dfneql(lhs: Term, rhs: Term, simp: Boolean = false) =
      DefnEqualityMixin(DefnEqual(lhs, rhs), this)

    def globaldfn(x: Term, y: Term, local: Boolean = true) =
      GlobalDefnMixin(Defn(termSymbol(x), y), this)

    def simpeql(lhs: Term, rhs: Term, simp: Boolean = false) =
      SimpEqualityMixin(DefnEqual(lhs, rhs), this)
  }
  ...
  case class LambdaMixin[+U <: Term with Subs[U], V <: Term with Subs[V]](
    ...
    def withNewTail(newtail: Context[Term, V]) =
      LambdaMixin(variable, newtail, dep)

    def foldin: Typ[V] => ConstructorType => V =
      (tp) => (f) => tail.foldin(tp)(f(variable))

    def symblist(tp: Typ[V])(varnames: List[AnySym]) =
      apply(tp).symbObj(varnames.head) :: tail.symblist(tp)(varnames.tail)
    ...
    def defnEqualities: Seq[DefnEquality] =
      tail.defnEqualities map ((dfn) => dfn.map(optlambda(variable)))
...
//    val typ = tail.typ
  ...
  case class DefnEqualityMixin[+U <: Term with Subs[U], V <: Term with Subs[V]](
    ...
    def withNewTail(newtail: Context[Term, V]) =
      DefnEqualityMixin(eqlty, newtail)
...
//    val typ = tail.typ
  ...
  case class SimpEqualityMixin[+U <: Term with Subs[U], V <: Term with Subs[V]](
    ...
    def withNewTail(newtail: Context[Term, V]) =
      SimpEqualityMixin(eqlty, newtail, dep)

    def foldin: Typ[V] => ConstructorType => V =
      (tp) => (f) => tail.foldin(tp)(f(eqlty.lhs))
