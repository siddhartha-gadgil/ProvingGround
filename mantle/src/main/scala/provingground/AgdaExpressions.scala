...
object AgdaExpressions {
  ...
  /**
    ...
    */
  class AgdaParse(patterns: List[List[String]] = List())
    ...
    /**
      ...
      */
    def appl(sp: Parser[Unit] = spc): Parser[Expression] =
      term ~ sp ~ expr ^^ { case f ~ _ ~ x => Apply(f, x) }
    ...
    /**
      ...
      * @param sp spaces, usually either a spaces/tabs or a whitespace.
      */
    def arrow(sp: Parser[Unit] = spc): Parser[Expression] =
      term ~ sp ~ To ~ sp ~ expr ^^ { case a ~ _ ~ _ ~ _ ~ b => Arrow(a, b) }

    /**
      * expression "(a : A) :-> b" parsing to lambda.
      */
    def lambda(sp: Parser[Unit] = spc): Parser[Expression] =
      typedvar ~ sp ~ mapsTo ~ sp ~ expr ^^ {
        case x ~ _ ~ _ ~ _ ~ y => LambdaExp(x, y)
      }

    /**
      * expression (a : A) -> B with B a function of a.
      */
    def deparrow(sp: Parser[Unit] = spc): Parser[Expression] =
      typedvar ~ sp ~ To ~ sp ~ expr ^^ {
        case x ~ _ ~ _ ~ _ ~ y => DepArrow(x, y)
      }

    /**
      * the first universe
      */
    def univ: Parser[Expression] = "_" ^^ { (_) =>
      U
    }
    ...
    private def ptnmatchlist(
      ...
      ptn match {
        ...
        case "_" :: tail =>
          term ~ recptnmatch(tail, sp) ^^ { case x ~ ys => x :: ys }
    ...
    def term: Parser[Expression] =
      "(" ~ opt(wspc) ~> expr <~ opt(wspc) ~ ")" | "begin" ~ wspc ~> expr <~ wspc ~ "end" | token
    ...
    /**
      * expression A = B
      */
    def eqlty(sp: Parser[Unit] = spc): Parser[Equality] =
      opt(spc) ~> expr ~ sp ~ "=" ~ sp ~ expr <~ opt(spc) ^^ {
        case lhs ~ _ ~ _ ~ _ ~ rhs => Equality(lhs, rhs)
      }
    ...
    /**
      ...
      */
    def data =
      "data" ~ spc ~> typdefn ~ spc ~ "where" ~ wspc ~ repsep(typdefn, wspc) <~ eop ^^ {
        case x ~ _ ~ _ ~ _ ~ ls => (x, ls)
      }
    ...
    /**
      ...
      */
    def defn = typdefn ~ wspc ~ repsep(eqlty(wspc), wspc) ^^ {
      case x ~ _ ~ y => (x, y)
    }
  }
  ...
  class AgdaPatternParser extends JavaTokenParsers {
    ...
    def prefixPtn: Parser[List[String]] =
      blank ~ mixfixPtn ^^ { case head ~ tail => head :: tail } | blank ^^ {
        List(_)
      }

    def mixfixPtn: Parser[List[String]] =
      word ~ prefixPtn ^^ { case head ~ tail => head :: tail } | word ^^ {
        List(_)
      }
