...
object Graph {
  ...
  case class Edge[A](
      initial: A, terminal: A, weight: Double = 1.0, oriented: Boolean = false)

  case class IndexEdge(
      init: Int, term: Int, weight: java.lang.Double, oriented: Boolean)
    ...
    def fromMap[V](map: Map[V, List[V]],
      ...
      val edges = for ((x, l) <- map; y <- l) yield
        Edge(x, y, weight, oriented)
      Graph(vertices.toSet, edges.toList)
    }

    def fromSeqs[V](
        ss: Seq[Seq[V]], weight: Double = 1.0, oriented: Boolean = false) = {
...
case class Graph[V](vertices: Set[V], edges: List[Edge[V]]) {
  ...
  val jEdges =
    edges map ((e) =>
          IndexEdge(
              index(e.initial), index(e.terminal), e.weight, e.oriented))
