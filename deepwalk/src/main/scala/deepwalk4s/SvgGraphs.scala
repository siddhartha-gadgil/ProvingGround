...
object SvgGraphs{
  ...
  def scatterPlot(points: Vector[(Double, Double, String)], width: Int = 1000, height: Int = 400, r: Double = 3, colour: String = "blue", fontSize: Int = 12) = {
      ...
      val circles = points map {case (x, y, t) => circ((x - xmin) * xScale + r + r, height -r -r - ((y - ymin) * yScale), t, r, colour, fontSize)}
      s"""
      <div>
      ${header(width, height)}
      ${circles.mkString("\n")}
