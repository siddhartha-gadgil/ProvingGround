...
object WordExpressions{
	...
	case class Sequence(ts: Stream[Term]) extends Term{
		...
		val freeVars = if (ts.hasDefiniteSize) (ts map (_.freeVars) reduce (_ union _)) else (Set.empty : Set[Var])
	}
	...
	private def subsWordFirst[A](l: List[A], first: List[A], second: List[A]): Set[List[A]] ={
	  val n = first.length
	  if (l.take(n) == first) Set((second ::: (l drop n))) else Set.empty
	}
	
	def subsWord[A](l: List[A], first: List[A], second: List[A]): Set[List[A]] ={
	  val tailSet: Set[List[A]] = if (l.isEmpty) Set() else subsWord(l.tail, first, second) 
	...
	def relationSet(rels: Set[List[Term]]) ={
	  rels flatMap ((r:List[Term]) => Set((r, List.empty : List[Term]), (List.empty : List[Term], r)))
	}
	
	def equalitySet(eqns: Set[(List[Term], List[Term])]) = eqns union symSet(eqns)
}
