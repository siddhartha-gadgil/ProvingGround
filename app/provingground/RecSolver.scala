...
object RecSolver {
  ...
  def solveSeq[P, S](as: Iterable[Either[P, S]])(implicit solvers: AtomSeq[P, S]): Option[S] = {
    if (as.isEmpty) None
    else as.head match {
      case Right(s) => Some(s)
  ...
  def trySolve[P, S](s: Either[P, S])(implicit solvers: AtomSeq[P, S]) = s match {
    case Left(a) => solve(a)(solvers) map (Right(_)) getOrElse (Left(a))
    case Right(a) => a
  }
  
  def bestNewSolutions[P, S](s: Either[P, S])(implicit solvers: AtomSeq[P, S]): Set[Either[P,S]] = {
    val nxt = for (sl <- solvers.atoms; p <- sl(s)) yield p
    bestSolutions(nxt.toSet)(solvers)
  }

  def bestSolutions[P, S](start: Set[Either[P, S]])(implicit solvers: AtomSeq[P, S]): Set[Either[P,S]] = {
	  start flatMap {s => if (bestNewSolutions(s)(solvers).isEmpty) Set(s) else bestNewSolutions(s)(solvers) }
  }
