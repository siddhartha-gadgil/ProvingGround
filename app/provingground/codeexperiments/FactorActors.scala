	...
	class FactorActor extends Actor{
		...
		def receive = {
		  ...
		  case PrimeFactor(p: BigInt, n: BigInt) =>
		    for (Query(Factorise(m: BigInt), asker: ActorRef) <- tasks if m==n) {
		      partialFactors += PartialFactorisation(n, List(p), n/p)
		      primeFinder ! FindPrimeFactor(n/p)
		    }
		    for (PartialFactorisation(m: BigInt, l: List[BigInt], unfactored: BigInt) <- partialFactors if unfactored == n){
		  ...
		  case IsPrime(n: BigInt) =>
		    ...
		    for (PartialFactorisation(m: BigInt, l: List[BigInt], unfactored: BigInt) <- partialFactors if unfactored == n){
		      ...
		      for (Query(Factorise(k: BigInt), asker: ActorRef) <- tasks if k==m) {asker ! Factorisation(n, n :: l)}
		    }
		  case TimedOut(task: ActorTask) => timedout += task
		  case Updates =>
		    for (Factorisation(n, l) <- factorisations if (timedout contains Factorise(n))) sender ! Factorisation(n,l)
			}
	}
	...
	class PrimeFactorFinder extends Actor{
	  ...
	  def receive = {
	    ...
	    case NoPrimeFactor(n: BigInt, lower: BigInt, upper: BigInt) =>
	      sender ! FindPrimeFactor(n, lower + increment, Some(upper + increment))
	    case  PrimeFactor(prime: BigInt, n: BigInt) =>
	      for (Query(FindPrimeFactor(m, _,_), asker) <- tasks if m==n) (asker ! PrimeFactor(prime, n))
	    case  IsPrime(n: BigInt) =>
