...
object Recursion{
  ...
  private def recContextChange[V<: Term](f : => (FuncLike[Term, Term]),
        W : Typ[Term], X : Typ[V]) : (AnySym, FmlyPtnLike[Term], Context[Term, V]) => Context[Term, V] = (varname, ptn, ctx) => {
  ...
  private def indContextChange[V<: Term](f : => (FuncLike[Term, Term]),
      W : Typ[V], Xs : Term => Typ[V]) : (AnySym, FmlyPtnLike[Term], Context[Term, V]) => Context[Term, V] = (varname, ptn, ctx) =>   {
    val x =  ptn(W).symbObj(varname)
   ...
   */
  private type Change[V <: Term] = (AnySym, FmlyPtnLike[Term], Context[Term, V]) => Context[Term, V]
