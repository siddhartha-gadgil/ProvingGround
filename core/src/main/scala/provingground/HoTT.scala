...
//      else SymbTyp(name, level - 1)
    ...
    case ft: FuncTyp[u, v] =>
      skolemize(ft.codom) match {
        ...
        case st: SigmaTyp[x, y] =>
          ...
          SigmaTyp(beta :-> skolemize(PiDefn(a, Q.replace(b, beta(a)))))
        case typ: Typ[x] => ft.dom ->: skolemize(typ)
      }
    case typ => typ
  }
  ...
  def fromSkolemized(typ: Typ[Term])(y: Term): Term = {
    require(
      y.typ == skolemize(typ),
      s"$y does not have skolemized type ${skolemize(typ)} of $typ"
    )
    typ match {
      case pd: ProdTyp[u, v] =>
        y match {
      ...
      case pd: PiDefn[u, v] =>
        skolemize(pd.value) match {
          case pt: ProdTyp[x, y] =>
            ...
            pd.variable :~>
              fromSkolemized(pd.value)(
                pair(fold(l1)(pd.variable), fold(l2)(pd.variable))
              )
          case st: SigmaTyp[x, y] =>
            val a    = pd.variable
            val b    = st.fib.variable
            ...
            pd.variable :~>
              fromSkolemized(pd.value)(
                mkPair(fold(l1)(pd.variable), fold(l2)(pd.variable))
              )
          case tp =>
...
//        (p: PairTerm[U, V]) => targetFmly(p.first)(p.second)
  ...
  case class PiSymbolicFunc[W <: Term with Subs[W], U <: Term with Subs[U]](
    ...
    lazy val dom: Typ[W] = variable.typ.asInstanceOf[Typ[W]]

    lazy val depcodom: W => Typ[U] = (arg: W) =>
      value.replace(variable, avoidVar(variable, arg))

    lazy val typ = PiDefn(variable, value)

    def act(arg: W): U = {
      val newarg = avoidVar(variable, arg)
      value.replace(variable, newarg).symbObj(ApplnSym(this, newarg))
    }
