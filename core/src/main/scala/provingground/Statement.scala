...
object Statement{
  ...
  abstract class Define[S, E : ExprLang](name: S, typ: E) extends Statement[E] with Assertion[E]{
    lazy val lang = implicitly[ExprLang[E]]
    
    lazy val variable = lang.variable(name, typ).get
  }
  
  case class Definition[S, E : ExprLang](name: S, typ: E, value: E) extends Define[S, E](name, typ){    
    lazy val claim = lang.equality(variable, value).get
  }
  
  case class DefnPropn[S, E: ExprLang](name: S, typ: E, claim: E) extends Define[S, E](name, typ)
