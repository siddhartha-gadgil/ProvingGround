...
object Context{
  ...
  case class AppendDefn[U <: Term with Subs[U]](init: Context, defn: Defn[U], global: Boolean) extends Context{
    ...
    val definitions =
      if (global) init.definitions :+ defn.map(init.export) else init.definitions
  ...
  case class AppendTerm[U <: Term with Subs[U]](init: Context, term: U, role: Role) extends Context{
    val constants = init.constants
...
trait Context{
  ...
  def define[U <: Term with Subs[U]](name: Term, value: U) = AppendDefn(this, Defn(name, value), true)

  def let[U <: Term with Subs[U]](name: Term, value: U) = AppendDefn(this, Defn(name, value), false)
  ...
  def introduce[U <: Term with Subs[U]](t: U, role: Role = Consider) = AppendTerm(this, t, role)
