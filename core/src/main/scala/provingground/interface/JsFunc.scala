...
object JsFunc {
  ...
  def toJs[I, F[_]](pat: Pattern[I, F])(
    name: String,
    header: String = "intro")(
    implicit
    jsF: JsFunc[F]): Translator[I, Js.Value] =
    pat >>> { (js) =>
  ...
  def jsToOpt[I, F[_]: Traverse](name: String, header: String = "intro")(
    build: F[I] => Option[I])(
    implicit
    jsF: JsFunc[F]): Translator[Js.Value, I] = {
...
object TermJson {
  ...
  def jsonToTerm(
    ...
    indexedInds: Term => Option[IndexedConstructorSeqDom[_, Term, _, _, _]] = (_) => None) =
    jsonToTermBase ||
