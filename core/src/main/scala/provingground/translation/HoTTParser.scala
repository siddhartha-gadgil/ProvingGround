...
case class HoTTParser(names: Map[String, Term] = Map()){self =>
  ...
  val alphachar: immutable.IndexedSeq[Char] = ('a' to 'z') ++ ('A' to 'Z') ++ Seq('$', '@', '_')

  val alphanum: immutable.IndexedSeq[Char] = alphachar ++ ('0' to '9') ++ Seq('.')

  val str: core.Parser[String, Char, String] = (P(CharIn(alphachar).!)~P(CharIn(alphanum).rep.!)).map{case (h, t) => h + t.toString}
