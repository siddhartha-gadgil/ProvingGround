...
trait CompositeFunctors {
   ...
   */
  implicit def traverseCompose[X[_]: Traverse, Y[_]: Traverse]: Traverse[({ type Z[A] = X[Y[A]] })#Z] =
...
object Functors extends CompositeFunctors {
   ...
   */
  implicit def traverseEquiv[F[_], Y[_]](implicit
    equiv: Equiv[F, Y],
  ...
  implicit def trCnst[X]: Traverse[({ type Z[A] = C[A, X] })#Z] =
    new Traverse[({ type F[A] = C[A, X] })#F] {
      ...
      def traverse[G[_], A, B](fa: X)(f: A => G[B])(
        implicit
        evidence$1: cats.Applicative[G]): G[X] =
