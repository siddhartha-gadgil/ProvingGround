...
QField.{w => _, x => _, y => _, z => _, Literal => rat, _},
...
//Proving that d(n) = c(n+1) with the value dIsc
  ...
  val dIsc = FreeGroup.rm(s.inverse |+| t |+| zwbarn |+| zwbar) *: (PowerDistributive
    .pf(wy)(nat(1))(n))
...
//get bounds for the expression for x^2n
  ...
  val exprBound = lemmaProof(n) + triang(s)(c(n)) + triang(s |+| c(n))(
    t.inverse)
