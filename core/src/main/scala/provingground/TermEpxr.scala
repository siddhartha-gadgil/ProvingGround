...
case class SigmaExpr(fiber: TermExpr) extends TermExpr {
  def asTerm(implicit lp: LiteralParser) = fiber.asTerm match {
    ...
    case _ =>
      throw (new IllegalArgumentException(s"cannot construct Sigma Type with fibre ${fiber.asTerm}"))
  }
...
case class PiExpr(fiber: TermExpr) extends TermExpr {
  def asTerm(implicit lp: LiteralParser) = fiber.asTerm match {
    ...
    case _ =>
      throw (new IllegalArgumentException(s"cannot construct Pi Type with fibre ${fiber.asTerm}"))
  }
