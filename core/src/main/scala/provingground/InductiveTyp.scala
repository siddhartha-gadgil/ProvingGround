...
trait InductiveTyp[C <: Term with Subs[C], H <: Term with Subs[H]] extends Typ[H]{
  ...
  def induc(Xs: Func[H, Typ[C]]) = ConstructorSeq.inducFn(constructors, this, Xs)
  /**
...
object InductiveTyp{
  def fromFormal(formalCons: List[Term], formalTyp : Typ[Term]) = {
      val constructorDefs =
        formalCons map ((cons) =>
        (typ: Typ[Term]) =>
        Constructor.fromFormal(cons, formalTyp)(typ)
      )
...
case class InductiveTypDefinition[C <: Term with Subs[C]](
      constructorDefs : List[Typ[Term] => Constructor[C, Term]]) extends InductiveTyp[C, Term]{
  val typ  = Type
 ...
 *  *  For convenience, these are usable as stub implementations.  */
  ...
  def withCod[CC <: Term with Subs[CC]] =
    InductiveTypDefinition(constructorDefs map ((fn) => (t: Typ[Term]) => (fn(t).withCod[CC])))
