...
object Inclusion{
  implicit def subtypIncl[X[_], Y[_] >: X[_]] : Inclusion[X, Y] = new Inclusion[X, Y]{
    def incl[I] = x => x
  }

  implicit def pairInclusion[X1[_], Y1[_], X2[_], Y2[_]](
    implicit incl1: Inclusion[X1, Y1], incl2: Inclusion[X2, Y2])  =
    new Inclusion[({type X[A] = (X1[A], X2[A])}) # X, ({type Y[A] = (Y1[A], Y2[A])}) # Y]{
...
object OptRestriction{
  implicit def subtypRestriction[X[_], Y[_] >: X[_]] : OptRestriction[X, Y] = new OptRestriction[X, Y]{
  ...
  implicit def pairRestriction[X1[_], Y1[_], X2[_], Y2[_], G[_]](
    ...
    new OptRestriction[({type X[A] = (X1[A], X2[A])}) # X, ({type Y[A] = (Y1[A], Y2[A])}) # Y]{
      def restrict[I] = _.flatMap {case (y1, y2) =>
        for (x1 <- rest1.restrict(Some(y1)); x2 <- rest2.restrict(Some(y2))) yield (x1, x2)}
    }
}

trait ContextTranslator[I, O, X[_], Ctx[_, _]] extends (Ctx[I, O] => X[I] => Option[X[O]]) { self =>
...
object ContextTranslator{
  /**
    ...
    * only information is the type parameters, so may be good starting point.
    */
  case class Empty[I, O, X[_], Ctx[_, _]]() extends ContextTranslator[I, O, X, Ctx] {
  ...
  /**
    * Tries the first translator at top level, then the second. Is recursive.
    */
  case class OrElse[I, O, X[_], Ctx[_, _]](first: ContextTranslator[I, O, X, Ctx],
    second: ContextTranslator[I, O, X, Ctx])
      extends ContextTranslator[I, O, X, Ctx] {
    def recTranslate(leafMap: => (Ctx[I, O] => X[I] => Option[X[O]])) =
      (ctx: Ctx[I, O]) =>
      (inp: X[I]) =>
        first.recTranslate(leafMap)(ctx)(inp) orElse second.recTranslate(leafMap)(ctx)(
          inp)
  }
  ...
  /**
    ...
    * These allow applying the recursive translator on the components.
    */
  case class Junction[I, O, X[_], Ctx[_, _], Y[_], Z[_]: Traverse](split: Ctx[I, O] => X[I] => Option[Y[I]],
    ...
    def recTranslate(leafMap: => (Ctx[I, O] => X[I] => Option[X[O]])) = {
      def connect(ctx: Ctx[I, O])(zi: Z[X[I]]) = flip(implicitly[Functor[Z]].map(zi)(leafMap(ctx)))
      (ctx: Ctx[I, O]) =>
      (inp: X[I]) =>
        rest.restrict(split(ctx)(inp) map (incl.incl) flatMap (connect(ctx)) ) flatMap (build(ctx))
    }
  }
