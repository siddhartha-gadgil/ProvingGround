...
sealed trait TypFamilyExst{
  ...
  def lambdaExst[TT<: Term with Subs[TT]](variable: TT, dom: Typ[TT]) =
    TypFamilyExst(
      DepFuncTypFamily(dom, (t: TT) => pattern.subs(variable, t)), variable :~> W
    )

  def ~>:[TT <: Term with Subs[TT]](variable: TT) =
    TypFamilyExst(
      DepFuncTypFamily(variable.typ.asInstanceOf[Typ[TT]], (t: TT) => pattern.subs(variable, t)), variable :~> W
    )
...
trait IndexedIterFuncExst{
  ...
  def piWrap[TT<: Term with Subs[TT]](variable: TT, dom: Typ[TT]) = IndexedIterFuncExst(piShape(variable, dom))
...
object IndexedIterFuncExst{
  ...
  def apply[Fm <: Term with Subs[Fm]](sh: IndexedIterFuncShape[Term, Fm, Fb, Index]) = new IndexedIterFuncExst{
    type F = Fm
    val shape = sh
  }
  ...
  def getIterFuncShape(fmly: Typ[Term]): IndexedIterFuncExst = fmly match {
    ...
    case tp : Typ[u] =>
      IndexedIterFuncExst(IdIterShape[Term, Fb, Index](pattern, pattern.getIndex(W, tp).get))
  }
...
object IndexedConstructorShapeExst{
  def apply[SI <: HList, ConstructorTypeI <: Term with Subs[ConstructorTypeI]](
    ...
    def getIndexedConstructorShape(cnstTyp: Typ[Term]): IndexedConstructorShapeExst =
      ...
      {
        cnstTyp match {
      case ft: FuncTyp[u, v] if (ft.dom.indepOf(W)) => ft.dom ->>: getIndexedConstructorShape(ft.codom)
      case pd: PiDefn[u, v] if (pd.domain.indepOf(W)) => pd.variable ~>>: getIndexedConstructorShape(pd.value)
      case ft: FuncTyp[u, v]  =>
...
object IndexedConstructorSeqExst{
  def apply[SSS<: HList, IIntros <: HList](seq: IndexedConstructorSeqDom[SSS, Term, Fb, Index, IIntros]) =
  ...
  def getIndexedConstructorSeq(intros: List[Term]) : IndexedConstructorSeqExst = {
    intros match {
      ...
      case l =>
        ...
        val head = name ::: IndexedConstructorShapeExst.getIndexedConstructorShape(x.typ)
...
object TypFamilyExst{
  import TypFamilyPtn._
  def apply[Fib <: Term with Subs[Fib], In <: HList : Subst](tf: TypFamilyPtn[Term, Fib, In ], w: Fib) =
    new TypFamilyExst{
      type Fb = Fib
      type Index = In
...
def getFamily[Fb <: Term with Subs[Fb]](w: Fb) : TypFamilyExst = w match {
  ...
  def getIndexedConstructorSeq[Fb <: Term with Subs[Fb]](w: Fb, intros: List[Term]) = {
