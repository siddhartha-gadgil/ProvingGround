...
object ConstructorPattern {
    ...
    constype match {
      ...
      case pt: GenFuncTyp[u, v] =>
        ...
        if (pt.domain.dependsOn(w)) {
          ...
          val tail = pt.domain
          val cp = CnstDepFuncPtn[u, egfib.RecDataType, egfib.InducDataType, Term, v, H](tail, headfibre)
   ...
   */
  sealed trait RecursiveConstructorPattern[Cod <: Term with Subs[Cod], ArgT <: Term with Subs[ArgT], HeadT <: Term with Subs[HeadT], CT <: FuncLike[ArgT, HeadT] with Subs[CT], H <: Term with Subs[H]]
  ...
  case class FuncPtn[C <: Term with Subs[C], F <: Term with Subs[F], HC <: Term with Subs[HC], H <: Term with Subs[H]](
    ...
    head: ConstructorPattern[C, HC, H]) extends RecursiveConstructorPattern[C, F, HC, Func[F, HC], H] { self =>
    //    type ArgType = F
  ...
  case class CnstFncPtn[T <: Term with Subs[T], Cod <: Term with Subs[Cod], HC <: Term with Subs[HC], H <: Term with Subs[H]](
    tail: Typ[T],
    head: ConstructorPattern[Cod, HC, H]) extends RecursiveConstructorPattern[Cod, T, HC, Func[T, HC], H] { self =>
    //   type ArgType = Term
  ...
  case class CnstDepFuncPtn[T <: Term with Subs[T], V <: Term with Subs[V], VV <: Term with Subs[VV], C <: Term with Subs[C], HC <: Term with Subs[HC], H <: Term with Subs[H]](
    tail: Typ[T],
    headfibre: T => (ConstructorPattern[C, HC, H] {
      type RecDataType = V; type InducDataType = VV
    }),
    headlevel: Int = 0) extends RecursiveConstructorPattern[C, T, HC, FuncLike[T, HC], H] {
    self =>
...
case class ConstructorDefn[U <: Term with Subs[U], C <: Term with Subs[C], H <: Term with Subs[H]](
  pattern: ConstructorPattern[C, U, H],
  cons: U,
  W: Typ[H]) extends Constructor[C, H] {
