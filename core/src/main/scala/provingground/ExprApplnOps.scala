...
class ExprApplnOps[E](appln : => (E, E) => Option[E])(implicit l : ExprLang[E], d: Domain[E], ep : ExprPatterns[E]) {
  ...
  def convert(cs: TruncatedDistribution[E => Option[E]])(f: E, x : E) = {
    ...
    val resOpts = argOptTD map ((argOpt) => argOpt flatMap ((arg) => appln(f, arg)))
