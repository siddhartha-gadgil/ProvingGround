...
object TangVec {
  ...
  implicit def vs[T](
    implicit
    bvs: VectorSpace[T, Double]): VectorSpace[TangVec[T], Double] =
...
object AdjDiffbleFunction {
  ...
  case class Composition[A, B, C](
    f: AdjDiffbleFunction[A, B],
    g: AdjDiffbleFunction[B, C]) extends AdjDiffbleFunction[A, C] {
  ...
  case class Oplus[A, B, C, D](
    ...
    second: AdjDiffbleFunction[C, D]) extends AdjDiffbleFunction[(A, C), (B, D)] {
  ...
  def block[A: LinearStructure, B: LinearStructure, C: LinearStructure, D: LinearStructure](
    f: AdjDiffbleFunction[A, C],
  ...
  case class BigSum[A: LinearStructure, B: LinearStructure](
    fns: A => Traversable[AdjDiffbleFunction[A, B]]) extends AdjDiffbleFunction[A, B] {
    val func = (a: A) => {
  ...
  implicit def diffFnLS[A: LinearStructure, B: LinearStructure]: LinearStructure[AdjDiffbleFunction[A, B]] = {
  ...
  implicit def vecSpaceDiffFn[A, B](implicit
    vsA: VectorSpace[A, Double],
    vsB: VectorSpace[B, Double]) =
