...
class FineEvolver(
  ...
  def evolvTypFamilies(fd: FD[Term]): PD[SomeFunc] =
    asFuncs {
      fd.<+?>(
        unifApplnEv(evolvTypFamilies, evolve)(fd),
...
class FineDeducerStep[X[_]](
  ...
  lazy val feedBacks: X[Vector[(FD[Term], Double)]] =
    for {
      ...
      thmFb <- thmFeedback
    } yield for { (x, (tfd, tpfd)) <- derFDs } yield (x, thmFb.feedbackTermDist(tfd, tpfd))
