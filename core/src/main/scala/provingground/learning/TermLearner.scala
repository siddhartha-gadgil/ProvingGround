...
/**
 * Elements associated to variables, for use in equations and entropies
 */
 abstract class VarElements[F : Field]{
   /**
    ...
    context match {
      ...
      case init :+ last =>
        termsInContext(terms, init).collect{
          case l: LambdaTerm[u, v] if l.variable.typ == last.typ => l.value.replace(l.variable, last)
          case pd: PiDefn[u, v] if pd.variable.typ == last.typ => pd.value.replace(pd.variable, last)
        }
    }
    ...
    term match {
      case l: LambdaTerm[u, v] =>
        val tail = contextsOfTerm(l.value)
         Vector(l.variable) +: tail.map{(ctx) => l.variable +: ctx}
     case l: PiDefn[u, v] =>
       val tail = contextsOfTerm(l.value)
        Vector(l.variable) +: tail.map{(ctx) => l.variable +: ctx}
      case t => Vector()
...
import EvolverEquations._
 /**
  * The equations for consistency of probabilities
  */
 abstract class EvolverEquations[F](implicit field: Field[F]) extends VarElements[F]{
  ...
  def totProbOne(context: Vector[Term]) : F =
   ...
   def hasTypProb(typ: Typ[Term], context: Vector[Term]) =
     totFinalProb(termSetInContext(context).filter(_.typ == typ), context) - hasTyp(typ, context)

  def isFuncProb(context: Vector[Term]) =
       totFinalProb(termSetInContext(context).filter((t) => isFunc(t)), context) - isFuncP(context)
