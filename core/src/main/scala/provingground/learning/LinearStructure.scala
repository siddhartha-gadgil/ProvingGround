...
object LinearStructure {
  ...
  def vBigSum[T](xs: Iterable[T])(implicit ls: LinearStructure[T]) = {
    xs.foldRight(ls.zero)(ls.sum)
  }

  def vAverage[T](xs: Iterable[T])(implicit ls: LinearStructure[T]) = {
  ...
  implicit val RealsAsLinearStructure: LinearStructure[_root_.scala.Double] =
  ...
  implicit def VectorPairs[A, B](
    ...
    def multpair(sc: Double, vect: (A, B)) =
      (lsa.mult(sc, vect._1), lsb.mult(sc, vect._2))
  ...
  implicit def FiniteDistVec[T]: LinearStructure[FiniteDistribution[T]] =
