...
object Unify {
    ...
    if (lhs == rhs) Some(Map())
      ...
      (lhs, rhs) match {
        ...
        case (PlusTyp(a: Typ[u], b: Typ[v]), PlusTyp(c: Typ[w], d: Typ[x])) =>
          unifyAll(freevars)(a -> c, b -> d)
        case (
            IdentityTyp(dom1: Typ[u], a: Term, b: Term),
  ...
  def subsApply(
    ...
    val x = Try(multisub(arg, unifMap)).fold(fa => {
      pprint.log(func)
      pprint.log(arg)
      pprint.log(unifMap)
      throw fa
    ...
    val lambdaVars = (freeVars filter ((x) => !(unifMap.keySet contains x)))
      .map(y => multisub(y, unifMap))
    import Fold._
    Try(polyLambda(lambdaVars.toList, fn(x))).toOption
  }

  case class UnifyApplnException(
      func: Term,
      arg: Term,
      freeVars: Vector[Term],
      error: Throwable
  ) extends Exception(
        s"Unified application error for $func on $arg, variables: $freeVars"
      )
  ...
  def purgedPairsList(
      fxs: List[(Term, Term)],
      accum: List[(Term, Term)] = List()
  ): List[(Term, Term)] =
    fxs match {
      case List() => accum
      case head :: tail =>
        val needHead = !(tail exists (
            (fx) =>
