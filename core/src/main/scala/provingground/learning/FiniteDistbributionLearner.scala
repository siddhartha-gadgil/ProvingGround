...
object FiniteDistributionLearner {
  ...
  case class NewVertex[V](v: V)
    extends AdjDiffbleFunction[(Double, FiniteDistribution[V]), FiniteDistribution[V]] {
   ...
   */
  def weightedDyn[M, X: LinearStructure: InnerProduct]: (M, AdjDiffbleFunction[X, X]) => AdjDiffbleFunction[(FiniteDistribution[M], X), X] =
    (m, fn) => {
  ...
  case class ExtendM[M, X](
    ...
    extends AdjDiffbleFunction[(FiniteDistribution[M], X), (FiniteDistribution[M], X)] {
   ...
   */
  def extendM[M, X](fn: AdjDiffbleFunction[(FiniteDistribution[M], X), X]): AdjDiffbleFunction[(FiniteDistribution[M], X), (FiniteDistribution[M], X)] = {
    ExtendM(fn)
  }
