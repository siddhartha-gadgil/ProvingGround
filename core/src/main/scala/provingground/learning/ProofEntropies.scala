...
case class ProofEntropies(gens: Map[Term, Double], theorems: Map[Typ[Term], Double], proofs: Map[Term, Double], genWeight: Double) {
  val genEnt: Double = gens.values.map((p) => - p * log(p)).sum

  val thmSet: Set[Typ[Term]] = theorems.keySet.intersect(proofs.keySet.map(_.typ))

  def proofWeight(thm: Typ[Term]): Double = proofs.collect{case (x, p) if x.typ == thm => p}.sum

  val kl: Double = thmSet.map{
    (thm) =>
      val p = theorems(thm)
  ...
  def addGen(x: Term, p: Double): ProofEntropies =
    this.copy(gens = addElem(x, p, gens), proofs = addElem(x, p * genWeight, proofs))
...
object ProofEntropies{
  ...
  def fromStates(init: TermState, result: TermState, genWeight: Double) =
    ProofEntropies(init.terms.toMap, result.thmsBySt.toMap, result.pfDist.toMap, genWeight)
}