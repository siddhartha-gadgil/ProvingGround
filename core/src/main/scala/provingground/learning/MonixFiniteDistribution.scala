...
abstract class GenMonixFiniteDistribution[State](
  ...
  def varDist[Y](initState: State)(
      ...
      epsilon: Double,
      limit: FiniteDuration = 3.minutes): Task[FD[Y]] =
...
//            .map(_.getOrElse(fullArg, FD.empty[Y]))
          varFamilyDistFunc(initState)(randomVarFmly, epsilon)(fullArg)
            .map(_.flatten.safeNormalized)
            .timeout(limit)
        case _ =>
          find(randomVar)
            .map { nc =>
              nodeCoeffDist(initState)(nc, epsilon).map(
                _.flatten.safeNormalized)
            }
...
/**
  ...
  */
case class MonixFiniteDistribution[State](
