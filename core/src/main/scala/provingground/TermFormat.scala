...
object FansiTranslate{
  ...
  def apply(x: Term) = fansiTrans(x) map (_.toString()) getOrElse(x.toString())

  // import fansi.Color.LightRed

  val fansiTrans =
    Translator.Empty[Term, Str]  ||
    formalAppln >>> {case (func, arg) => func ++ "(" ++ arg ++ ")"} ||
    funcTyp >>> {case (dom , codom) => dom ++ " " ++ Color.LightRed(syms.Arrow) ++ " " ++ codom} ||
    lambdaTriple >>> {case ((variable, typ), value) => (Str("(") ++ variable ++ Yellow(" : ") ++ typ ++ ") " ++ LightRed(
        syms.MapsTo) ++ " " ++ value)} ||
    piTriple >>> {case ((variable, typ), value) => (Cyan(Str(syms.Pi)) ++ LightYellow("(") ++ variable ++ Yellow(" : ") ++ LightRed(
        "{ ")++ value ++ LightRed(" }")) } ||
    sigmaTriple >>> {case ((variable, typ), value) => (Cyan(Str(syms.Sigma)) ++ LightYellow("(") ++ variable ++ Yellow(" : ") ++ LightRed(
        "{ ")++ value ++ LightRed(" }")) } ||
    universe >>> {(n) => (LightCyan(Str(UnivSym)))} ||
    symName >>> ((s) => (Str(s))) ||
    absPair >>> {case (first, second) => (Str("(") ++ first ++ " , " ++ second ++ ")")} ||
    ...
    plusTyp >>> {case (first, scnd) => (first ++ LightRed(Str(" + ")) ++ scnd)}
...
object FansiShow {
  ...
  implicit def tuple3[U1 : FansiShow,
                      U2 : FansiShow,
                      U3 : FansiShow]: FansiShow[(U1, U2, U3)] =
