...
object Z3Examples{
   ...
   *  checking injectivity in the i-th coordinate;
  ...
  def CommAxiom(ctx: Context, f: FuncDecl) = {
        val t = f.Range();
        val dom = f.Domain();

        assert(dom.length == 2)

        val bench = ("(benchmark comm :formula (forall (x " + t.Name()
                + ") (y " + t.Name() + ") (= (" + f.Name() + " x y) ("
                + f.Name() + " y x))))")
        ctx.ParseSMTLIBString(bench, Array( t.Name()),
                Array(t), Array(f.Name()),
                Array(f));
        ctx.SMTLIBFormulas()(0);
    }
  ...
  def Check(ctx: Context, f: BoolExpr, sat: Status) = {
    ...
    if (s.Check != sat) None
      ...
      if (sat == Status.SATISFIABLE) Some(s.Model) else None
    }
  }
  ...
  def SudokuExample(ctx: Context) = {
    
    // 9x9 matrix of integer variables
    val Xexp = (1 to 9).toArray map ((i : Int) =>
    			(1 to 9).toArray map ((j: Int) =>
    			  ctx.MkConst(
                        ctx.MkSymbol("x_" + i + "_" + j),
                        ctx.IntSort) 
                        )) 
    
    val asArith:  Expr => ArithExpr = {case exp: ArithExpr => exp}
    
    val X = Xexp map ((row) => row map asArith)
    
    val cells_c = (0 to 8).toArray map ((i : Int) =>
    				(0 to 8).toArray map ((j: Int) =>
    					ctx.MkAnd(Array(ctx.MkLe(ctx.MkInt(1), X(i)(j)),
    					    ctx.MkLe(X(i)(j), ctx.MkInt(9))
                        ))))
    val rows_c = (0 to 8).toArray map ((i: Int) =>
    				ctx.MkDistinct(X(i) map (asArith)))
  
    val Xtrans = (0 to 8).toArray map ((i: Int) =>
      				(0 to 8).toArray map ((j: Int) =>
      				  X(j)(i)))
    
    val cols_c = (0 to 8).toArray map ((i: Int) =>
    				ctx.MkDistinct(Xtrans(i) map (asArith)))
    
    def square(i0: Int, j0: Int) = (((X drop (3*i0)) take 3) map ((row) => (row drop (j0*3)) take 3)).flatten
    
    val sq_c = (0 to 2).toArray map ((i: Int) =>
      				(0 to 2).toArray map ((j) =>
      				  ctx.MkDistinct(square(i, j) map (asArith)))
        )
   
    val sudoku_c = ctx.MkAnd(Array(ctx.MkAnd(cells_c.flatten), ctx.MkAnd(rows_c), ctx.MkAnd(cols_c), ctx.MkAnd(sq_c.flatten)))
    
    def matchGrid(grid: Array[Array[Int]]) = {
      val all = for (i<- 0 to 8; j <- 0 to 8) yield (ctx.MkITE(ctx.MkEq(ctx.MkInt(grid(i)(j)), ctx.MkInt(0)), ctx.MkTrue, 
    		  									ctx.MkEq(X(i)(j), ctx.MkInt(grid(i)(j)))))  
      ctx.MkAnd(all map (_.asInstanceOf[BoolExpr]) toArray) 
    }
    ...
    val instance_c = matchGrid(instance)
