...
object WikiRead{
  ...
  def masterpage = MathPage("Lists of mathematics topics", baseUrl+"/wiki/Lists_of_mathematics_topics")
  ...
  def nextgen(pgsfut: Future[Seq[MathPage]]) = pgsfut map (_ filter (_.isList)) flatMap ((pgs) => 
    Future.sequence(
        pgs map ((pg) => 
      getXML(pg.url) map ((xml) => mathPages(xml))
      ))
        ) map (_.flatten)
