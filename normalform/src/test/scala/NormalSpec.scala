...
class MainStuctureSpec extends FlatSpec {
  ...
  "A number" should "add with a number" in {
    ...
    assert(a+b === result)
    assert(Sigma(CommTerm(Map((a, 1)))) + Sigma(CommTerm(Map((b, 1)))) === result)
  ...
  it should "multiply with a sigma" in {
    val a = NumTerm(4)
    val b = Sigma(AssocTerm(List(NumTerm(3), Variable("x"))))
    val result = Sigma(AssocTerm(List(NumTerm(12), Pi(CommTerm(Map((NumTerm(4), 1), (Variable("x"), 1)))))))
    assert(a*b === result)
  }
  ...
  "A sigma" should "add with a sigma" in {
    ...
    val result1 = Sigma(CommTerm(Map((Variable("x"), 2), (Variable("z"), 1), (Variable("y"), 1))))
    assert(a+b === result1)
  }
  ...
  it should "multiply with a sigma" in {
    ...
    val x = Variable("x")
    val y = Variable("y")
    val result = Sigma(CommTerm(Map((Pi(CommTerm(Map((x, 2)))), 1), (Pi(CommTerm(Map((x, 1), (y, 1)))), 2), (Pi(CommTerm(Map((y, 2)))), 1))))
    assert(a*a === result)
  }
  ...
  it should "add with a pi" in {
    ...
    val result = Sigma(CommTerm(Map((Variable("x"), 1), (Variable("y"), 1), (Pi(CommTerm(Map((Variable("x"), 1), (Variable("z"), 1)))), 1))))
    assert(a+b === result)
  }
  ...
  it should "multiply with a pi" in {
    val a = Sigma(CommTerm(Map((Variable("x"), 1), (Variable("y"), 1))))
    val b = Pi(CommTerm(Map((Variable("x"), 1), (Variable("z"), 1))))
    val result = Sigma(CommTerm(Map((Pi(CommTerm(Map((Variable("x"), 2), (Variable("z"), 1)))), 1), (Pi(CommTerm(Map((Variable("x"), 1), (Variable("y"), 1), (Variable("z"), 1)))), 1))))
    assert(a*b === result)
  }
  ...
  "A Pi" should "multiply with a Pi" in {
    ...
    val result = Pi(CommTerm(Map((Variable("x"), 2), (Variable("z"), 1), (Variable("y"), 1))))
    assert(a*b === result)
  }
}
...
class AuxiliaryFunctionSpec extends FlatSpec {
  ...
  "numTermMatch" should "return true for NumTerm and false for everything else" in {
    val a = NumTerm(4)
    val b = Variable("x")
  ...
  "clumpTerms" should "clump together adjacent items in a list based on whether they satisfy a predicate" in {
  ...
  "commReduce" should "eagerly reduce all the numeric terms in a CommRep[Term] to a single Term" in {
    val a = CommTerm(Map((NumTerm(3), 2), (NumTerm(2), 1)))
    val b = CommTerm(Map((Variable("x"), 2), (Variable("y"), 3)))
    val c = CommTerm(Map((NumTerm(18), 1), (Variable("x"), 2), (Variable("y"), 3)))
  ...
  "assocReduce" should "eagerly reduce all adjacent numeric terms in an AssocRep[Term] to as few terms as possible" in {
