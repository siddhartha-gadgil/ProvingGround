...
object EquationDistanceFromIndexed{
      /**
  ...
  /**
    * all matches of indices satisfying a condition, typically having a good enough distance bound
    *
...
class EquationDistanceFromIndexed(
    rhsIndexedExprs: Vector[SumIndexExpression],
    probVec: Vector[Double],
    lhsExpressions: Vector[Expression]
) extends BackTraceFromIndexed(rhsIndexedExprs, probVec, lhsExpressions) {
    import EquationDistanceFromIndexed._
