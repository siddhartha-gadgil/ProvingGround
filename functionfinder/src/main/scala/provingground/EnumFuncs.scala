...
object EnumFuncs {
  def allMaps[U, V](dom: List[U], codom: List[V]): List[Map[U, V]] =
    dom match {
      ...
      case x :: zs =>
        for (y <- codom; m <- allMaps(zs, codom)) yield (m + (x -> y))
    }
  ...
  def allSecMaps[U, V](dom: List[U], codoms: U => List[V]): List[Map[U, V]] =
    dom match {
      ...
      case x :: zs =>
        for (y <- codoms(x); m <- allSecMaps(zs, codoms)) yield (m + (x -> y))
    }

  def allSecMapsOpt[U, V](
      dom: List[U],
      codoms: U => Option[List[V]]): Option[List[Map[U, V]]] =
    dom match {
      ...
      case x :: zs =>
        ...
        for (l <- codopt; tail <- tailsecsopt)
          yield (for (y <- l; m <- tail) yield (m + (x -> y)))
    }

  def pairs[U <: Term with Subs[U], V <: Term with Subs[V]](first: List[U],
                                                            second: List[V]) =
    for (x <- first; y <- second) yield PairTerm(x, y)

  def allPairs[U <: Term with Subs[U], V <: Term with Subs[V]](
      dom: List[U],
      cods: U => Option[List[V]]): Option[List[AbsPair[U, V]]] =
    dom match {
      case List() => Some(List())
      case x :: List() =>
        for (l <- cods(x)) yield (for (y <- l) yield PairTerm(x, y))
      case x :: zs => {
        val heads = for (l <- cods(x))
          yield (for (y <- l) yield PairTerm(x, y))
        for (h <- heads; t <- allPairs(zs, cods)) yield (h ++ t)
      }
    }

  def allFunc[U <: Term with Subs[U], V <: Term with Subs[V]](
      domenum: EnumTerm[U])(codomenum: EnumTerm[V]) = {
    val maps = for (f <- allMaps(domenum.value, codomenum.value))
      yield new FuncDefn(f, domenum.elemTyp, codomenum.elemTyp)
    EnumTerm(maps, domenum.elemTyp ->: codomenum.elemTyp)
  }

  def EnumFunc[U <: Term with Subs[U], V <: Term with Subs[V]](
      dom: Typ[U],
      codom: Typ[V]) = {
  ...
  def allSec[U <: Term with Subs[U], V <: Term with Subs[V]](
      domenum: EnumTerm[U])(codomenums: U => EnumTerm[V])(
      implicit sv: ScalaUniv[V]) = {
    val maps =
      for (f <- allSecMaps(domenum.value, (u: U) => codomenums(u).value))
        yield depFunc(domenum.elemTyp, f)
  ...
  def EnumSec[U <: Term with Subs[U], V <: Term with Subs[V]](
      dom: Typ[U],
      codoms: U => Typ[V])(implicit su: ScalaUniv[U], sv: ScalaUniv[V]) = {
  ...
  val enumSec = lambda(A)(lambda(Bs)(EnumSec(A, Bs)))
}
