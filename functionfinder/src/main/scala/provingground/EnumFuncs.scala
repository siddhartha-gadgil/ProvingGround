...
object EnumFuncs {
  ...
  def allSecMapsOpt[U, V](
      dom: List[U], codoms: U => Option[List[V]]): Option[List[Map[U, V]]] =
    dom match {
  ...
  def allFunc[U <: Term with Subs[U], V <: Term with Subs[V]](
    ...
    val maps = for (f <- allMaps(domenum.value, codomenum.value)) yield
      new FuncDefn(f, domenum.elemTyp, codomenum.elemTyp)
    EnumTerm(maps, domenum.elemTyp ->: codomenum.elemTyp)
  }

  def EnumFunc[U <: Term with Subs[U], V <: Term with Subs[V]](
      dom: Typ[U], codom: Typ[V]) = {
  ...
  def allSec[U <: Term with Subs[U], V <: Term with Subs[V]](
    ...
    val maps = for (f <- allSecMaps(
        domenum.value, (u: U) => codomenums(u).value)) yield
  ...
  def EnumSec[U <: Term with Subs[U], V <: Term with Subs[V]](
      dom: Typ[U], codoms: U => Typ[V])(
      implicit su: ScalaUniv[U], sv: ScalaUniv[V]) = {
